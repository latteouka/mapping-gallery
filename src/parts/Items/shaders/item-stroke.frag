...
void main(void) {
  ...
  stroke += (smallnoise) + (bignoise);


  light += (bignoise) * light;

  stroke  = smoothstep(light - 0.5, light + 2.0, stroke);
  float stroke2  = 1.0 - smoothstep(light - 2.0, light + 2.0, stroke);

  gl_FragColor = vec4(vec3(stroke2), 1.0);
}
